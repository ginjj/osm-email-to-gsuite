...
def check_authentication() -> Optional[str]:
    """
    Check authentication and return user email if authenticated.
    
    Returns:
        User email if authenticated, None otherwise
    """
    # Check if we have authentication info in session state
    if 'user_email' in st.session_state:
        return st.session_state['user_email']
    
    # In Cloud Run, check for authentication headers
    # Note: This requires Cloud Run to be configured with IAP or similar
    if os.getenv('K_SERVICE'):
        # For now, we'll use a simple authentication prompt
        # In production, you'd integrate with Google Identity Platform or IAP
        return None
    
    # In development mode, simulate authentication
    return "dev@1stwarleyscouts.org.uk"
...
def show_login_page():
    ...
    st.info("👤 **Please enter your @1stwarleyscouts.org.uk email address to continue**")
    
    with st.form("login_form"):
        email = st.text_input(
            "Email Address",
            placeholder="your.name@1stwarleyscouts.org.uk",
            help="Enter your 1st Warley Scouts email address"
        )
        ...
        if submitted:
            ...
            elif not email.endswith(f"@{AUTHORIZED_DOMAIN}"):
                st.error(f"❌ Invalid email domain. You must use an @{AUTHORIZED_DOMAIN} email address.")
            else:
                # Check if user is in the authorized group
                with st.spinner("Verifying authorization..."):
                    if check_user_authorization(email):
    ...
    with st.expander("ℹ️ Troubleshooting & Help"):
        ...
        **"Invalid email domain" error:**
        - Make sure you're using your **@{AUTHORIZED_DOMAIN}** email
        - Personal Gmail accounts (@gmail.com) are not allowed
        
        **"Access Denied" error:**
        - Your account exists but isn't in the authorized group
        - Contact an administrator to request access
        - They need to add you to: `{AUTHORIZED_GROUP}`
        ...
        **For Administrators:**
        ...
        #### Security Note:
        This is a basic authentication system. In production with sensitive data,
        consider using Google Identity Platform or Identity-Aware Proxy (IAP) for
